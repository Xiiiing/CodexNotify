...
const {settings,hook,installHook,selectStorage,migrateStorage,testBark,setSecret}=vi.hoisted(()=>{
  ...
  return{settings,hook,installHook:vi.fn().mockResolvedValue({...hook,trusted:false,trustStatus:"untrusted",reviewRequired:true}),selectStorage:vi.fn().mockResolvedValue(undefined),migrateStorage:vi.fn().mockResolvedValue(undefined),testBark:vi.fn().mockResolvedValue({ok:true,elapsedMs:42}),setSecret:vi.fn().mockResolvedValue(undefined)};
});
vi.mock("./api",()=>({api:{state:vi.fn().mockResolvedValue({storage:{configured:true,mode:"default",root:"/tmp/CodexNotify",configDir:"/tmp/config",dataDir:"/tmp/data",logDir:"/tmp/logs",locatorFile:"/tmp/storage.json"},settings,counts:{queued:0,sending:0,retrying:0,sent:4,failed:0,suppressed:0},secrets:{barkKeyConfigured:true,encryptionKeyConfigured:false},hook,health:{}}),events:vi.fn().mockResolvedValue([]),autostart:vi.fn().mockResolvedValue(false),installHook,selectStorage,migrateStorage,testBark,setSecret,save:vi.fn().mockResolvedValue(undefined),deleteSecret:vi.fn().mockResolvedValue(undefined),uninstallHook:vi.fn().mockResolvedValue(hook),hookStatus:vi.fn().mockResolvedValue(hook)}}));
