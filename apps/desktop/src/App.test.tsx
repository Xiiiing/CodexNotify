...
const {settings,hook,installHook,selectStorage,migrateStorage,testBark,testHook,setSecret,uninstallApplication}=vi.hoisted(()=>{
  ...
  return{settings,hook,installHook:vi.fn().mockResolvedValue({...hook,trusted:false,trustStatus:"untrusted",reviewRequired:true}),selectStorage:vi.fn().mockResolvedValue(undefined),migrateStorage:vi.fn().mockResolvedValue(undefined),testBark:vi.fn().mockResolvedValue({ok:true,elapsedMs:42}),testHook:vi.fn().mockResolvedValue({ok:true,elapsedMs:55,deliveryStatus:"sent",errorCode:"",message:""}),setSecret:vi.fn().mockResolvedValue(undefined),uninstallApplication:vi.fn().mockResolvedValue(undefined)};
});
vi.mock("./api",()=>({api:{state:vi.fn().mockResolvedValue({storage:{configured:true,mode:"default",root:"/tmp/CodexNotify",configDir:"/tmp/config",dataDir:"/tmp/data",logDir:"/tmp/logs",locatorFile:"/tmp/storage.json"},settings,counts:{queued:0,sending:0,retrying:0,sent:4,failed:0,suppressed:0},secrets:{barkKeyConfigured:true,encryptionKeyConfigured:false},hook,health:{status:"success",deliveryStatus:"sent",lastAttemptAt:"2026-09-02T00:00:00Z"}}),events:vi.fn().mockResolvedValue([]),autostart:vi.fn().mockResolvedValue(false),installHook,selectStorage,migrateStorage,testBark,testHook,setSecret,uninstallApplication,save:vi.fn().mockResolvedValue(undefined),deleteSecret:vi.fn().mockResolvedValue(undefined),uninstallHook:vi.fn().mockResolvedValue(hook),hookStatus:vi.fn().mockResolvedValue(hook),diagnostics:vi.fn()}}));
...
test("complete removal requires typing the product name",async()=>{render(<App/>);fireEvent.click(await screen.findByText("System"));fireEvent.click(await screen.findByText("Remove application"));const confirm=screen.getByLabelText("Type CodexNotify to confirm");const remove=screen.getAllByText("Remove application").at(-1)!.closest("button")!;expect(remove).toBeDisabled();fireEvent.change(confirm,{target:{value:"CodexNotify"}});expect(remove).toBeEnabled();fireEvent.click(remove);await waitFor(()=>expect(uninstallApplication).toHaveBeenCalledTimes(1));});
